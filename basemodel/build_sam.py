...
from basemodel.modeling.image_encoder import ImageEncoderViT
from basemodel.modeling.mask_decoder import MaskDecoder
from basemodel.modeling.prompt_encoder import PromptEncoder
from basemodel.modeling.sam import Sam
from basemodel.modeling.transformer import TwoWayTransformer
...
def load_from(sam, state_dict, image_size, vit_patch_size, encoder_global_attn_indexes):
    ...
    new_state_dict = {k: v for k, v in state_dict.items() if k in sam_dict.keys() and except_keys[0] not in k and except_keys[1] not in k and except_keys[2] not in k}
